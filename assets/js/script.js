  ...
  for (let i = 0; i <= lyrics.length - 1; i++) {
    if (
      ...
      for (let j = 0; j <= lyrics[i].data.length - 1; j++) {
        let str = ` <span style="color: white;font-weight: bold;" id='span_${j}'>
        ...
        for (let l = 0; l <= lyric_replace.length; l++) {
          ...
          if (
            ...
            {
              $(`#span_${j}`).css("color", "red"); // color red by the time in lyric song === current time
            }
          }
          if(lyric_replace.length === l && (Math.ceil(audio.currentTime) === Math.ceil(37.939)) ){
            $(`#span_${lyric_replace.length+1}`).css("color", "red")
          }
        }
      }
    }
